...
use config::CONFIG;
use enclave_server_template::logger::init_logger;
use log::info;
use commands::{
    activate_e3_round, decrypt_and_publish_result, initialize_e3_round,
...
fn select_action() -> Result<usize, Box<dyn std::error::Error + Send + Sync>> {
    ...
    Ok(FuzzySelect::with_theme(&ColorfulTheme::default())
        .with_prompt("Create a new E3 round or participate in an existing round.")
        .default(0)
        .items(&selections[..])
        .interact()?)
}
