...
pub async fn initialize_e3_round() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    info!("Starting new E3 round!");
...
pub async fn participate_in_existing_round(client: &Client) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let resp = client.post(&url)
        .json(&PKRequest { round_id: input_e3_id, pk_bytes: vec![0] })
        .send()
        .await?;
    ...
    if let Some(vote) = vote_choice {
        ...
        let contract = EnclaveContract::new(CONFIG.enclave_address.clone()).await?;
        let res = contract
            .publish_input(U256::from(input_e3_id), Bytes::from(ct.to_bytes()))
            .await?;
...
pub async fn decrypt_and_publish_result(client: &Client) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let resp = client.post(&url)
        .json(&CTRequest { round_id: input_e3_id, ct_bytes: vec![0] })
        .send()
        .await?;
    ...
    let params_bytes = db.get(format!("e3:{}", input_e3_id))?.ok_or("Key not found")?;
    ...
    let contract = EnclaveContract::new(CONFIG.enclave_address.clone()).await?;
    let res = contract
        .publish_plaintext_output(U256::from(input_e3_id), Bytes::from(votes.to_be_bytes()))
        .await?;
    info!("Vote broadcast. TxHash: {:?}", res.transaction_hash);
